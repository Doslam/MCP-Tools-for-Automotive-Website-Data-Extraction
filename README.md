...
2. 将已实现的 MCP 工具注册到 LangChain Agent  
3. Agent 根据用户指令自动规划调用路径并选择合适工具  
4. MCP 工具在真实浏览器上下文中执行页面访问、滚动、展开评论与信息抽取  
5. 工具返回**结构化数据结果**，作为 Agent 推理链中的中间输入  
6. LangChain Agent 对多工具结果进行整合，生成最终响应  
